...
func load_templates(template_data_array: Array[Dictionary]) -> void:
	templates.clear()
	for data in template_data_array:
		if data and data.has("spell_name") and data.has("strokes"):
...
func recognize(raw_strokes: Array[PackedVector2Array]) -> String:
	...
	for template in templates:
		var template_strokes = _json_to_strokes(template["strokes"])
		
		if candidate.size() != template_strokes.size():
			continue
			
		var template_processed = _process(template_strokes)
...
func _json_to_strokes(json_strokes: Array) -> Array[PackedVector2Array]:
	var result: Array[PackedVector2Array] = []
	for stroke_data in json_strokes:
		var packed_stroke = PackedVector2Array()
		for pt in stroke_data:
			packed_stroke.append(Vector2(pt[0], pt[1]))
		result.append(packed_stroke)
	return result
