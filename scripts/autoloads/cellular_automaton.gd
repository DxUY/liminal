...
#func _initialize_rendering() -> void:
	...
	#
	#_recreate_texture(800, 600) # Default size, can be adjusted dynamically
...
#func _dispatch_simulation(delta: float) -> void:
	## Default dispatch groups for an 800x600 grid (update if size changes)
	#var groups := Vector2i((800 + 7) / 8, (600 + 7) / 8)
#
